...
public class SystemInitHandler {
    ...
    private void executeCommand(String command) throws ParseException {
        ...
        else if (splitedCommand[0].equals("addSlot")) {//[String courseID, int groupID, int slotID, int day, int hour, int duration]
            Slot s = new Slot(splitedCommand[1], Integer.valueOf(splitedCommand[2]), Integer.valueOf(splitedCommand[3]), Integer.valueOf(splitedCommand[4]), Integer.valueOf(splitedCommand[5]), Integer.valueOf(splitedCommand[6]));
            Controller.create(s);
        }
        else if (splitedCommand[0].equals("addSlotCapacity")) {//[int slotID, int capacity]
            SlotCapacity ac = new SlotCapacity(Integer.valueOf(splitedCommand[1]), Integer.valueOf(splitedCommand[2]));
        }
        else if (splitedCommand[0].equals("addSlotDate")) {//[int slotID, Date date]
            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            SlotDates sd = new SlotDates(Integer.valueOf(splitedCommand[1]), df.parse(splitedCommand[2]));
        ...
        else if (splitedCommand[0].equals("addSchedule")){//[String studentID, int slotID]
            Schedule s = new Schedule(splitedCommand[1], Integer.valueOf(splitedCommand[2]));
            Controller.create(s);
        }
        else if (splitedCommand[0].equals("addAssignings")){//[int slotID, String studentID, Date date]
            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
